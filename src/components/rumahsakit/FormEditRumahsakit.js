...
const FormEditRumahsakit = ({ open, setOpen, rumahsakit, refreshData, ...rest }) => {
  ...
  const findRumahsakitById = async () => {
    const findRumahsakit = await axios.get(
      `http://localhost:1337/rumahsakit/${rumahsakit._id}`
    );
    return findRumahsakit;
  };
      ...
      {open ? (
        <Container style={{ padding: 16, display: 'block' }}>
          ...
          <Box>
            ...
            >
              Form Edit {rumahsakit.name}
            </Typography>
            <Formik
              initialValues={{
                name: rumahsakit.name,
                address: rumahsakit.address,
                phone: rumahsakit.phone,
                map: rumahsakit.map
              }}
              ...
              onSubmit={async ({ name, address, phone, map }) => {
                ...
                const findRumahsakit = await findRumahsakitById()

                if (findRumahsakit.data.success === 1) {
                  const response = await axios.put(
                    `http://localhost:1337/rumahsakit/${findRumahsakit.data.rumahsakit._id}`,
                    {
                      name,
                      address,
                      phone,
...
export default FormEditRumahsakit;
